# Zadanie 6 - Algorytm ewolucyjny

Program szuka maksimum funkcji dwoch zmiennych:
...
Dla obu zmiennych przyjeto zakres `0 <= x < 2π` oraz `0 <= y < 2π`.
Z samego wzoru wynika gorna granica `8`, bo kazda z dwoch sum zawiera cztery skladniki trygonometryczne.
...
Algorytm wykonuje klasyczne operacje genetyczne:

- `krzyzowanie` - z dwoch rodzicow powstaja dwa nowe osobniki: `[x1, (y1+y2)/2]` oraz `[(x1+x2)/2, y2]`.
- `mutacja` - kazda wspolrzedna moze zostac zmieniona losowo o `+0.001` albo `-0.001`.
- `selekcja` - z populacji rodzicow i dzieci wybierane jest kolejne pokolenie metoda turniejowa z zachowaniem dwoch najlepszych osobnikow.

Domyslne parametry:

- populacja: `100` osobnikow
- liczba pokolen: `300`
- pary krzyzowane w pokoleniu: `50`
- prawdopodobienstwo mutacji wspolrzednej: `0.35`

## Wynik

Program wypisuje najlepszy znaleziony punkt `x`, `y` oraz wartosc `f(x,y)`.

![Wykres optimum](wykres.png)
